...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cs149_vec_int exp;
  __cs149_vec_float result;
  __cs149_mask maskAll, maskNot, mask;
  __cs149_vec_int one = _cs149_vset_int(1);
  __cs149_vec_float oneF = _cs149_vset_float(1.f);
  __cs149_vec_float ninedot = _cs149_vset_float(9.999999f); 
  ...
  for (i=0; i<N-VECTOR_WIDTH; i+=VECTOR_WIDTH){
    ...
    // if (exp[i]>0), then mask[i]=1
    _cs149_vgt_int(mask, exp, zero, maskAll); 
    
    maskNot = _cs149_mask_not(mask);  

    _cs149_vmove_float(result, val, maskAll);
    _cs149_vmove_float(result, oneF, maskNot);
    ...
    while (_cs149_cntbits(mask) != 0){
      _cs149_vmult_float(result, result, val, mask);
      _cs149_vsub_int(exp, exp, one, mask);
    ...
    _cs149_vgt_float(mask, result, ninedot, maskAll);
    _cs149_vmove_float(result, ninedot, mask);
